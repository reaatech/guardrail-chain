...
import type { LoadConfigOptions, LoadedConfig } from './types.js';
...
export function loadConfigFromEnv(prefix = 'GUARDRAIL_CHAIN'): Partial<LoadedConfig> {
  ...
  if (maxLatency || maxTokens || skipSlow !== undefined) {
    result.budget = {
      maxLatencyMs: maxLatency ? Number.parseInt(maxLatency, 10) : 1000,
      maxTokens: maxTokens ? Number.parseInt(maxTokens, 10) : 4000,
